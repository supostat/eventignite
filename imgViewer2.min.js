...
(function($) {
  $.widget('wgm.imgViewer2', {
    options: { zoomStep: 0.5, zoomMax: undefined, zoomable: true, dragable: true, onClick: $.noop, onReady: $.noop },
    _create: function() {
      var self = this;
      ...
      $img
        .one('load', function() {
          ...
          self.map.on('resize', function() {
            ...
            self.map.options.minZoom = self.map.getBoundsZoom(
              L.latLngBounds(L.latLng(0, 0), L.latLng(self.img.naturalHeight, self.img.naturalWidth)),
              true
            );
    ...
    _view_resize: function() {
      if (this.ready) {
        ...
        $view.css({ top: vTop + 'px', left: vLeft + 'px', width: width + 'px', height: height + 'px' });
      }
    },
    destroy: function() {
    ...
    getZoom: function() {
      if (this.ready) {
        ...
        if (constraint == 'width') {
          ...
          return Math.max(
            1,
            (width / (bounds.getEast() - bounds.getWest()) + height / (bounds.getNorth() - bounds.getSouth())) / 2
          );
        }
      } else {
        return null;
